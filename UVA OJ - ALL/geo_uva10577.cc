...
int main(){
  ...
  while(cin>>n,n){
    ...
    AchaCentroCircunferencia3Pontos(A,B,C,centro);

    double R = dist(A,centro);

    double ymax = -1e40,xmax = -1e40,ymin = 1e40,xmin = 1e40;

    double angle = (2.0*PI)/n; // 360/n so que em rad

    // double offset = atan2(centro.y-A.y,centro.x-A.x);

    // atan2 é a mesma coisa que essa atan aqui em baixo

    double offset = atan((A.y-centro.y)/(A.x-centro.x));


    // pra onde existe um ponto.... coloca um ponto lá é roda o resto
    // poligono se formará de qlqer forma, colocando isso
    for(int i = 0 ; i < n ; ++i){

      double x = centro.x + cos(angle*i+offset)*R;
      double y = centro.y + sin(angle*i+offset)*R;
    ...
    cout <<"Polygon "<< ct ++ <<": " << (xmax-xmin)*(ymax-ymin) << '\n';
  }
  return 0;
}
