...
bool angleCmp(ponto a, ponto b){    // funcao para ordernar os pontos
  ...
  double d1x = a.x - pivo.x,d1y = a.y - pivo.y;
  double d2x = b.x - pivo.x,d2y = b.y - pivo.y;
...
// perimetro do poligono convexo
double perimetro(vector<ponto> &P){
  double p = 0.0;
...
int main(){
  ...
  while(cin>>n,n){
    ...
    vector<ponto> ch = CH(pts);

    double per = perimetro(ch);
    ...
    cout << "Perimeter length = " << per << endl;
