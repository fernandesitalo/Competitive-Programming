...
inline bool le(point &p){
  if(cin>>p.x>>p.y>>p.z) return true;
  return false;
}
...
struct coisa{
  ...
  bool operator < (coisa other){
    if(l == other.l){ // pontos alinhados
      return d < other.d;
    }
    return l < other.l;
  }
...
int main(){
  ...
  while(cin>>n,n){
    ...
    for(int i = 0 ; i < n ; ++i){
      le(p);
      coisa aux;
      pointsToLine(point(0,0),p,aux.l);
      aux.d = dist(point(0,0),p);
      aux.p = p;
      C[qd(p)].pb(aux);
    }
    ...
    for(int i = 0 ; i < 4 ; ++i){
      ...
      for(int j = 1 ; j < (int)C[i].size() ; ++j){
        if(C[i][j-1].l == C[i][j].l && C[i][j-1].p.z >= C[i][j].p.z){
          ans.pb(C[i][j].p);
          C[i][j].p.z = C[i][j-1].p.z;
        }
      }
    }
    ...
    if((int) ans.size() == 0){
      ...
      for(int i = 0 ; i < (int)ans.size() ; ++i){
        cout << "x = " << ans[i].x << ", y = " << ans[i].y<< (i == (int)ans.size()-1?'.':';') << '\n';
      }
    }
  }
