...
struct cubo{
  quadrilatero xz,xy,zy;

  cubo(){}
  cubo(int x, int y,int z, int a){
    xz = quadrilatero(ponto(x,z),ponto(x+a,z+a));
    xy = quadrilatero(ponto(x,y),ponto(x+a,y+a)); // ok
    zy = quadrilatero(ponto(z,y),ponto(z+a,y+a));
  }
};
...
int main(){
  ...
  while(cin>>n,n > 0){
    ...
    for(int i = 1 ; i < n ; ++i){
      ...
      if(!SobrePosicao(its.xy,b.xy,aux.xy)){
        zero = true;
      }
      if(!SobrePosicao(its.xz,b.xz,aux.xz)){
        zero = true;
      }
      if(!SobrePosicao(its.zy,b.zy,aux.zy)){
        zero = true;
      }
    ...
    if(zero) cout << 0 << "\n";
      ...
      int vol = (its.xy.upperRight.x - its.xy.lowerLeft.x)*(its.xy.upperRight.y - its.xy.lowerLeft.y)*(its.xz.upperRight.y - its.xz.lowerLeft.y);
