...
bool trianguloEhObtuso(double a,double b, double c){

  double aa = a*a,bb = b*b,cc = c*c;

  double A = acos((-aa+bb+cc)/(2*b*c));
  double B = acos((-bb+aa+cc)/(2*a*c));
  double C = M_PI - A - B;
  double angle120 = DEG_to_RAD(120.0);
  return (cmp(A,angle120) >= 0 ||cmp(B,angle120) >= 0 || cmp(C,angle120) >= 0);
}
...
int main(){
  ...
  for(int i = 0 ; i < t ; ++i){
    ...
    double rr = 0;
    // if(trianguloEhObtuso(a,b,c) || trianguloEhObtuso(b,c,a)|| trianguloEhObtuso(c,a,b)){
      // rr = max(max(a,b),c)/2.0;
    // }

    double aa = a*a,bb = b*b,cc = c*c;
    if(aa > bb+cc) rr = a/2.0;
    else if(bb > cc+aa) rr = b/2.0;
    else if(cc > aa+bb) rr = c/2.0;
    else{
      rr = RaioCirculoForaDoTriangulo(a,c,b);
    }

    // cout << "==================================\n";
    // cout << max(max(a,b),c)/2.0 << "          " << RaioCirculoForaDoTriangulo(a,c,b) << endl;
