...
struct segmento{
  ...
  segmento(point aa, point bb){
    a = aa;
    b = bb;
    pointsToLine(aa,bb,l);
  }
  segmento(){}
};

// point a e o superior direito
// point b e o inferior esquerdo
bool entre(point p, point a, point b){
  return fabs((dist(a,p) + dist(p,b))-dist(a,b)) < EPS;
}

bool intersepta(segmento s1, segmento s2){
  point p;
  if(areSame(s1.l,s2.l) && (entre(s2.a,s1.a,s1.b) || entre(s2.b,s1.a,s1.b) || entre(s1.a,s2.a,s2.b) || entre(s1.b,s2.a,s2.b))) return true;
  if(areIntersect(s1.l,s2.l,p) == false) return false;
  return (entre(p,s1.a,s1.b) && entre(p,s2.a,s2.b));
}
...
int main(){
  ...
  for(;t--;){
    ...
    int colisao[q+2];
    memset(colisao,0,sizeof colisao);

    for(int i = 0 ; i < q; ++i){
      for(int j = i+1 ; j < q ; ++j){
        if(intersepta(L[i],L[j])){
          colisao[i]++;
          colisao[j]++;
        }
      }
