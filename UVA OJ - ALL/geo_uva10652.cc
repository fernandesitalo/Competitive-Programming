...
int main(){
  ...
  while(t--){
    ...
    for(int i = 0 ; i < n ; ++i){
      ...
      ponto t[] = {ponto(w/2.0,h/2.0),ponto(-w/2.0,h/2.0),ponto(w/2.0,-h/2.0),ponto(-w/2.0,-h/2.0)};
      areaTabuleiros += w*h;

      for(int j = 0 ; j < 4 ; ++j){
        ponto a = rotate(t[j],-theta);
        a.x += x;
        a.y += y;
        P.pb(a);
      }
    ...
    double areaCH = area(P);

    // areaCH ----- 100%
    // areaTab ----- x%
    // x = areaTabuleitos*100/areaCH

    cout << areaTabuleiros*100/areaCH << " %\n";
  }
