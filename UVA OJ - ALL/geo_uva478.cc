...
bool dentroDoPoligono(ponto pt, const vector<ponto> &P){
  ...
  for(int i = 0 ; i < (int) P.size() - 1 ; ++i){
    if(collinear(pt,P[i],P[i+1])) return false;
...
struct circulo{
  ...
  ponto c;
  double r;
  circulo(int idx,ponto c, double r) :idx(idx), c(c) , r(r) {}
  circulo(){}
};
...
struct poly{
  int idx;
  vector<ponto> P;
  poly(int idx,vector<ponto> Po) : idx(idx) , P(Po){}
};
...
int main(){
  ...
  while(cin>>tipo, tipo != "*"){
    ...
    if(tipo == "r"){
      ...
      ponto A,B;
      le(A),le(B);
      Po[0] = ponto(min(A.x,B.x),min(A.y,B.y));
      Po[2] = ponto(max(A.x,B.x),max(A.y,B.y));
      Po[1] = ponto(Po[0].x,Po[2].y);
      Po[3] = ponto(Po[2].x,Po[0].y);
      Po[4] = Po[0];
      V.pb(poly(cont++,Po));
    }else if(tipo == "c"){
      ponto A; double R;
      le(A);
      cin>>R;
      C.pb(circulo(cont++,A,R));
    }else{
      vector<ponto> Po(4);
      le(Po[0]),le(Po[1]),le(Po[2]);
      Po[3] = Po[0];
      V.pb(poly(cont++,Po));
    }
  }

  ponto flag(9999.9,9999.9);
  ...
  while(le(A),!(A == flag)){
    ...
    for(int i = 0 ; i < (int)V.size() ; ++i){
      if(dentroDoPoligono(A,V[i].P)){
        ans.insert(V[i].idx);
      }
    }

    for(int i = 0 ; i < (int)C.size() ; ++i){
      if(dentroCirculo(A,C[i].c,C[i].r) == 0){
        ans.insert(C[i].idx);
      }
    }

    if(ans.size() == 0){
      cout << "Point "<<idxP<<" is not contained in any figure\n";
    }else{
      for(auto i : ans){
        cout << "Point "<<idxP<<" is contained in figure "<<i<<'\n';
      }
    }
