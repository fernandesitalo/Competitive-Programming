...
bool ccw(ponto p, ponto q, ponto r){
  return cross(FazVetor(p,q),FazVetor(p,r))/2 > -EPS;
}

bool collinear(ponto p, ponto q, ponto r){
...
inline bool le(ponto &p){
  if(cin>>p.x>>p.y) return true;
  return false;
}

inline void imprime(ponto p){
  cout << p.x<<" " << p.y;
}
...
double cross(ponto o, ponto a, ponto b) {
...
int det3pontos(ponto p, ponto a, ponto b){
...
/*
    ...
    paralelogramo formado pelo dois vetores fornecidos
...
// VERIFICA SE O PONTO ESTA NO POLIGONO CONVEXO
...
int main(){
  ...
  while(1){
    ...
    int ans = 0;
    if(A==z&&B==z&&C==z) break;


    int xmin = min((int)ceil(A.x),min((int)ceil(B.x),(int)ceil(C.x)));
    xmin =  max(xmin,1);

    int xmax = max((int)floor(A.x),max((int)floor(B.x),(int)floor(C.x)));
    xmax = min(xmax,99);

    int ymin = min((int)ceil(A.y),min((int)ceil(B.y),(int)ceil(C.y)));
    ymin =  max(ymin,1);

    int ymax = max((int)floor(A.y),max((int)floor(B.y),(int)floor(C.y)));
    ymax = min(ymax,99);
    ...
    for(int i = xmin ; i <= xmax ; ++i){
      for(int j = ymin ; j <= ymax ; ++j){
        ...
        // int a = det3pontos(P,A,B);
        // int b = det3pontos(P,A,C);
        // int c = det3pontos(P,B,C);
