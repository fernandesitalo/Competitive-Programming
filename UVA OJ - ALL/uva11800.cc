...
struct ponto{
  ...
  bool operator == (const ponto outro) const {
    return (fabs(x-outro.x) < EPS && (fabs(y-outro.y)) < EPS);
      ...
      inline ponto operator + (ponto p) {
        return ponto(x + p.x, y + p.y);
    }
    
    // produto escalar 
    double operator *(const ponto &o) const { return x * o.x + y * o.y; }
    
    // Determinante, produto cruzado
    double operator ^(const ponto &o) const { return x * o.y - y * o.x; }
    
    ponto operator *(const double &m) const { return ponto(m * x, m * y); }
    
    ponto operator /(const double &m) const { return ponto(x / m, y / m); }
...
// Faz o produto escal de dois vetores
...
inline bool parallel(vetor a, vetor b){
    return cross(a, b) == 0;
}
...
vector<ponto> ptos(4);
inline  bool compare(ponto a, ponto b) {
    vetor v1 = FazVetor(ptos[0], a);
    vetor v2 = FazVetor(ptos[0], b);
    return atan2(v1.y, v1.x) < atan2(v2.y, v2.x);
}

void tipoQuadrilatero() {
    sort(ptos.begin(), ptos.end());
    sort(ptos.begin() + 1, ptos.end(), compare);

    bool isReto = produtoEscalar(FazVetor(ptos[0], ptos[3]), FazVetor(ptos[0], ptos[1])) == 0;
    double d[4];
    
    for (int i = 0; i < 4; ++i)
        d[i] = dist(ptos[i], ptos[(i + 1) % 4]);

    if (d[0] == d[1] && d[1] == d[2] && d[2] == d[3])
        if (isReto) printf("Square\n");
        else printf("Rhombus\n");
    else if (d[0] == d[2] && d[1] == d[3])
        if (isReto) printf("Rectangle\n");
        else printf("Parallelogram\n");
    else if (parallel(FazVetor(ptos[0], ptos[1]), FazVetor(ptos[3], ptos[2])) ||
       parallel(FazVetor(ptos[0], ptos[3]), FazVetor(ptos[1], ptos[2])))
        printf("Trapezium\n");
...
int main(){
	...
	int ct = 1;
	while(t--){
		for(int i = 0 ; i < 4 ; ++i) le(ptos[i]);		
		
		//~ for(int i = 0 ; i < 4 ; ++i){
			//~ cout << ptos[i].x << "   " << ptos[i].y << endl;
		//~ }
		cout << "Case " << ct++ << ": "; 
