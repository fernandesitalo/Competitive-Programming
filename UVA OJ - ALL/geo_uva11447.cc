...
int main(){
  ...
  while(t--){
    ...
    cin>>n;
    vector<ponto> P(n);
    for(int i = 0 ; i < n ; ++i) le(P[i]);
    P.push_back(P[0]);

    double W;
    cin>>W;
    double porcI,consumo,ganho;

    cin>>porcI>>consumo>>ganho;

    double vol = area(P)*W;
    porcI /= 100.0;
    double volumeI = vol*porcI;

    bool a = false;

    if(cmp(consumo,volumeI) > 0){
      cout << "Lack of water.";
      a = true;
      volumeI = 0.0;
    }else{
    ...
    double porc = 100.0*volumeI/vol;
    if(a) cout << " ";
    cout << "Final percentage: " << (int)floor(porc) << "%\n";
