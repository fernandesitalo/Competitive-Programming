...
vector<ponto> MONOTONECHAIN(vector<ponto> P){
  sort(P.begin(),P.end());
  ...
  for(int i = 0 ; i < (int)P.size() ; ++i){
    ...
    while((int)ch.size() >= 2 && det3pontos_ori(P[i],ch[(int)ch.size()-1],ch[(int)ch.size()-2]) < 0) ch.pop_back();
  ...
  for(int i = P.size()-1 ; i >= 0 ; --i){
    while((int)ch.size() >= casco && det3pontos_ori(P[i],ch[(int)ch.size()-1],ch[(int)ch.size()-2]) < 0) ch.pop_back();
...
int main(){
  ...
  while(cin>>n,n){
    ...
    // deixar um lado vertical e pegar o valor (maxX - minX)

    for(int i = 0 ; i < (int) P.size() - 1 ; ++i){

      // lado P[i] e P[i+1]
      // tenho que deixa-lo vertical

      ponto vet; // vetor que forma o lado

      if( P[i].y <= P[i+1].y) vet = (P[i+1] - P[i]);
      else vet = (P[i] - P[i+1]);


      double theta = atan2(vet.y,vet.x); // angulo de inclinacao do vetor
      theta = PI/2.0 - theta;

      double maxX = -1e20,minX = 1e20;

      for(int j = 0 ; j < (int) P.size()-1 ; ++j){
        ponto p = rotate(P[j],theta);
        maxX = max(maxX,p.x);
        minX = min(minX,p.x);
      }
