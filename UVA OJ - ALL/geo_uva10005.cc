...
int insideCircle(point p, point c, double r){
  ...
  return cmp(Euc,rSq) == -1 ? 0 : fabs(Euc-rSq) < EPS ? 1 : 2;
}


void cicle2PtsRad(point p1,point p2,double r, point &c){
...
int main(){
  ...
  while(cin>>n,n){
    ...
    for(int i = 0 ; i < n ; ++i){
      for(int j = 0 ; j < n ; ++j){
          if(i == j) continue;
          cicle2PtsRad(V[i],V[j],R,centro);
          bool po = true;
          for(int k = 0 ; k < n ; ++k){
            if(dist(centro,V[k]) - R >  EPS){ po = false; break; }
          }
          if(po){ o = false;goto A;}
      }
    }
    ...
    if(!o || n == 1)cout << "The polygon can be packed in the circle.\n";
