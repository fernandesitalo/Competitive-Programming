...
#define grava freopen("out","w",stdout);
...
double perimeter(vector<ponto> P){
	double p = 0.0;
	for(int i = 0 ; i < (int)P.size()-1 ; ++i){
			p += dist(P[i],P[i+1]);
	}
...
int main(){
	...
	while(t--){
		...
		double K = perimeter(P)/(double)(m-1);
		...
		for(int i = 0 ; i < (int)P.size()-1 ; ++i){
			...
			while(cmp(d+ant,K) >= 0){
				...
				if(ant){
					d -= (K-ant);
					ant = 0;
					vetor aux = scale(FazVetor(P[i],P[i+1]),(D - d) / D);
					ans.pb(translate(P[i],aux));
				}else{
					d -= K;
					vetor aux = scale(FazVetor(P[i],P[i+1]),(D - d) / D); 
					ans.pb(translate(P[i],aux));
