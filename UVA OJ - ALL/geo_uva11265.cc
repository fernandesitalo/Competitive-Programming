...
int main(){
  ...
  while(cin>>N>>W>>H){
    le(fonte);
    vector<ponto> P;
    P.push_back(ponto(0,0));
    P.push_back(ponto(W,0));
    P.push_back(ponto(W,H));
    P.push_back(ponto(0,H));
    P.push_back(P[0]);
    ...
    for(int i = 0 ; i < N ; ++i){
      ...
      if(ccw(a,b,fonte)){ // porra, a fonte ta pra lá(esquerda de ab) ou pra cá(direita de ab)
        P = cutPolygon(a,b,P);
      }else{
        P = cutPolygon(b,a,P);
      }
