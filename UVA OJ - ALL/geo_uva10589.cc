...
bool dentro(point p){
  return (cmp(C.x,p.x) >= 0 && cmp(C.y,p.y) >= 0 && cmp(p.x,A.x) >= 0 && cmp(p.y,A.y) >= 0);
}
  ...
  while(cin>>n>>a, n || (int)a){
    ...
    for(int i = 0 ; i < n ;++i){
      ...
      if(dentro(p)){
        ...
        // cout << insideCircle(p,A,a) << " " <<insideCircle(p,B,a) << " "<< insideCircle(p,C,a) << " " << insideCircle(p,D,a) << endl;

        if(insideCircle(p,A,a) <= 1){
          if(insideCircle(p,B,a) <= 1){
            if(insideCircle(p,C,a)<=1){
              if(insideCircle(p,D,a)<=1){
                m++;
              }}}}
      }
    }
