...
/* Dados dois pontos (p1 e p2) sobre um circulo e o raio do mesmo,
* encontra (se existir) dois centros para este circulo.
*/
bool circle2PtsRad(ponto p1, ponto p2, double r, ponto &c) {
	
	
    double dist_c2ab = sqrt(r*r-dist(p1, p2)*dist(p1, p2)/4);
    ...
    return true;
} // Para achar o outro centro, basta inverter p1 e p2
	...
	while(t--){
			...
			for(int i = 0 ; i <  n ; ++i){
				for(int j = i ; j <  n ; ++j){
					...
					if(!circle2PtsRad(P[i],P[j],R,centro)){continue;}
					
					int cont = 0;
					for(int k = 0 ; k < n ; ++k){
						if(dist(centro,P[k]) <= 2.5+EPS){
							cont++;
						}
					}
					ans = max(ans,cont);
					if(!circle2PtsRad(P[j],P[i],R,centro)){continue;}
